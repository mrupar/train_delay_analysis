...
```{r}
train_delay_per_day <- delays %>% 
  group_by(date=date(timestamp), train = train_no) %>% 
  summarise(n_train_delays = n(), .groups = 'drop')

train_delay_per_day <- train_delay_per_day %>%
  group_by(date) %>% summarise(delays = sum(n_train_delays)) %>%
  inner_join(train_delay_per_day) %>%
  mutate(perc = n_train_delays/delays)
```

In this time there were on average `r mean(delays_per_day$n)` .
